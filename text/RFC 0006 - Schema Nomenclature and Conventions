### Blueprints (_was: Plates or Templates)
...
### Modules (_was: Recipes_)
Modules are instructions to update the Zesty.io Manager interface and can also include Plugins, Collections, and Contents. 


### Plugins (_was: Recipes_)
Modules is a set of instructions to build a Content Structure in Zesty.io, which includes at least one Collection that requires routing and may include multiple Collections. They may include HTML and Parsley template files, style sheets, javascript files, and media references. Modules may be installed at the initial creation from a blueprint or any time after.  
